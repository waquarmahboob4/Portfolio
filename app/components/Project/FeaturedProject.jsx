     ...
     rounded-br-2xl dark:bg-dark dark:border-light lg:flex-col lg:p-8 xs:rounded-2xl xs:rounded-br-3xl xs:p-4 '>
      ...
      xs:-right-2 sm:h-[102%] xs:w-full xs:rounded-[1.5rem]"/>
        ...
        className='w-1/2 cursor-pointer overflow-hidden rounded-lg lg:w-full'>
            <AnimatedImages src={img} alt={title} className='w-full h-auto' whileHover={{scale:1.05}} transition={{duration:0.2}} priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 50vw"/>
        </Link>
        <div className='w-1/2 flex flex-col items-start justify-between pl-6 lg:w-full lg:pl-0 lg:pt-6'>
            ...
            <h2 className='my-2 w-1/2 text-left text-4xl font-bold dark:text-light sm:text-sm' >{title}  </h2>
        </Link>
